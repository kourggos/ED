...
void criaReg(FILE *fp, FILE *arqNomes, FILE *arqTexto) {
...
int main(void) {
    ...
    fpreg = fopen("registros.bin", "wb");
    fpnomes = fopen("nomes.txt", "r");
    fptxt = fopen("registros.txt", "w");

    for (int i=0; i<10; i++) {
        criaReg(fpreg, fpnomes, fptxt);
    }
    
    fflush(fpreg);

    copiaArquivo("registros.bin", "heapRegistros.bin");
    // copiaArquivo("registros.bin", "hashRegistros.bin");
    copiaArquivo("registros.bin", "bplusRegistros.bin");
